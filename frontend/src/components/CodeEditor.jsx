...
function CodeEditor() {
  ...
  const handleJoinSession = () => {
    ...
    setHasJoined(true);
    // Conectar después de obtener el nombre
    initializeSocket();
  };
  const initializeSocket = () => {
    ...
    socketRef.current.on('file-updated', ({ fileName, content }) => {
      ...
      if (activeFile?.name === fileName) setActiveFile(prev => ({ ...prev, content }));
    });
    ...
    socketRef.current.on('user-joined', ({ name }) => {
      showNotification(`${name} se unió a la sesión`, '#28a745');
    });
    socketRef.current.on('user-left', ({ name }) => {
      showNotification(`${name} salió de la sesión`, '#ffc107');
    });
