...
io.on('connection', (socket) => {
  ...
  socket.on('cursor-position', ({ sessionId, fileName, position }) => {
    const session = sessions.get(sessionId);
  ...
  socket.on('rename-file', ({ sessionId, oldName, newName }) => {
    const session = sessions.get(sessionId);
    if (session && session.files.has(oldName)) {
      const file = session.files.get(oldName);
      file.name = newName;
      session.files.delete(oldName);
      session.files.set(newName, file);
      io.to(sessionId).emit('file-renamed', { oldName, newName });
    }
  });
...
function generateUserColor(socketId) {
  ...
  // Usar el hash del socketId para obtener un color consistente
  let hash = 0;
